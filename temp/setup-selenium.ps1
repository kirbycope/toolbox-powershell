...
function Unzip-File ($archive, $directory) {
    # Pre-Event Message
    ...
    # Post-Event Message
    Write-Host " => File(s) extracted into $directory" -ForegroundColor Green
}
...
function Get-LatestDownloadUrlFirefoxDriver {
    ...
    $GeckoDriverUrl = "https://github.com/mozilla/geckodriver/releases/download/$Tag/geckodriver-$Tag-win32.zip"
...
function Get-LatestDownloadUrlIeDriver {
    ...
    # Navigate to the page and find the latest download link
    $DownloadPageUrl = "https://selenium.dev/downloads/"
    $InternetExplorerDriverServerUrl = (Invoke-WebRequest -UseBasicParsing $DownloadPageUrl).Content | %{[regex]::matches($_, '(?:https://selenium-release.storage.googleapis.com/)(.*)(?:">)').Groups[2].Value}
    $InternetExplorerDriverServerUrl = "$InternetExplorerDriverServerUrl" -replace '">'
...
Unzip-File $ChromeDriverZip $SeleniumDirectory
...
Unzip-File $FirefoxDriverZip $SeleniumDirectory
...
Unzip-File $InternetExplorerDriverServerZip $SeleniumDirectory
